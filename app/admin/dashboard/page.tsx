...
export default function Dashboard() {
  ...
  const [selectedOrderDeleteId, setSelectedOrderDeleteId] = useState<
    string | null
  >(null);
  ...
  const filterOrdersByPaymentStatus = (orders: Order[], status: string) => {
    switch (status) {
      ...
      case "cancelled":
        return orders.filter((order) => order.paymentStatus === "Cancelled");
      case "all":
      default:
        return orders;
    }
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!selectedService) validationErrors.push("Service selection is required");
    ...
    if (!carType) validationErrors.push("Car type is required");
    if (!orderSource) validationErrors.push("Order source is required");
    if (!paymentMethod) validationErrors.push("Payment method is required");
    if (!paymentUpdate) validationErrors.push("Payment update status is required");
  
    // If validation errors exist, display them separately
    if (validationErrors.length > 0) {
      validationErrors.forEach((error) => toast.error(error)); // Display each error separately
      return;
    }
          ...
          <div className="bg-white rounded-lg p-6">
            ...
            <div className="flex flex-col space-y-4 mb-4">
              ...
              <div className="flex space-x-4">
                ...
                <Button
                  onClick={() => handlePaymentStatusFilterChange("cancelled")}
                  className={
                    paymentStatusFilter === "cancelled"
                      ? "bg-black text-white whitespace-nowrap rounded-[8px] px-5 py-2"
                      : ""
                  }
                >
                    ...
                    <div className="  w-full flex flex-col md:flex-row gap-6 justify-center">
                      ...
                      >
                        ...
                        >
                          ...
                          <option value="Refreal">Refreal</option>
                        </select>
                        <select
                          id="paymnentMethod"
                          className="block bg-[#F7F8FA] px-4 py-4 w-[90%] md:w-full outline-gray-500 rounded-[8px]"
