...
export const sendOrderMail = async (
  ...
  const totalAmount = price + addOn.reduce((sum, addon) => sum + addon.price, 0) + tips;

  const addOnHTML = addOn.length
    ? `<p><strong>Add-ons:</strong></p>
       <ul>
         ${addOn.map((addon) => `<li>${addon.title} - ₹${addon.price}</li>`).join("")}
       </ul>`
...
<body>
  <div class="email-container">
    ...
    <div class="content">
      <h2>Dear ${name},</h2>
      <p>Thank you for your order! We are excited to serve you and ensure the best service experience possible.</p>
